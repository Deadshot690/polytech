  ...
  return (
    <group ref={group}>
      ...
      <mesh position={[0, 0, 0]}>
        ...
        <meshStandardMaterial color={color} roughness={0.2} metalness={0.5} emissive={new THREE.Color(color)} emissiveIntensity={0.1} />
      </mesh>

      {/* rings */}
      <mesh rotation={[Math.PI / 2.2, 0, 0]} position={[0, 0, 0]}>
        <torusGeometry args={[1.6, 0.12, 16, 128]} />
        <meshStandardMaterial color={color} roughness={0.6} metalness={0.1} transparent opacity={0.95} side={THREE.DoubleSide} />
      </mesh>
      <mesh rotation={[Math.PI / 2.6, 0.6, 0]} position={[0, 0, 0]}>
        <torusGeometry args={[2.1, 0.06, 8, 128]} />
        <meshStandardMaterial color={color} roughness={0.7} metalness={0.05} transparent opacity={0.75} side={THREE.DoubleSide} />
      </mesh>
      ...
      <group ref={orbitRef}>
        {[0, 1, 2, 3, 4, 5].map((i) => {
          ...
          return (
            <mesh key={i} position={[Math.cos(angle) * radius, Math.sin(angle) * radius * 0.6, 0.05 + i * 0.02]}>
